...
export default async (
  ...
  if (recoveredAddressFromMessage.toLowerCase() !== address.toLowerCase()) {
    return res.status(400).send({
      error: 'Address recovered form message/signature does not match address given',
    })
  }
  ...
  try {
    ...
    // TODO: Add Secure;
    return res.status(200).setHeader('Set-Cookie', `authToken="${authToken}"; HttpOnly;`).send({ message: 'Successfully signed in'})
