...
This lambda was initiated using AWS SAM CLI and [Envek template](https://github.com/Envek/aws-sam-typescript-layers-example).

Run locally using:
```bash
overmind start
# If don't have overmind
brew install tmux overmind
```

If you want live updates when working locally also, in a separate window, run
```bash
yarn run watch # Any changes to template.yml requires restarting overmind
```

Hit endpoints on `http://localhost:3000`

## Below are details for SAM template
